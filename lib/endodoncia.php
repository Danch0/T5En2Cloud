...
class Endodoncia{
    ...
    // return $this->mydb->getAll('endodoncia_ma');
    return $this->mydb->getAll('endodoncia_ma');
	}

	public function getRegistro($id) {
		//Manadamos llamar la funcion getRegistro('NombreTabla', array('id_tabla' => $id)) de la clase myDB()
    return $this->mydb->getRegistro('endodoncia_ma',array('id_endodoncia' => $id));
	}

	public function delete($id) {
		$consulta = $this->mydb->delete('endodoncia_ma',array('id_endodoncia' => $id));
	  if ($consulta == 1)
    ...
    $estado = $this->mydb->post('endodoncia_ma', $data);
	  if ($estado != 0)
	...
	public function put($req, $id) {
		...
		$estado = $this->mydb->put('endodoncia_ma',array('id_endodoncia'=>$id), $data);
	  if ($estado != 0)
